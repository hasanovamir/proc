...
error_t read_commands (context_t* context, const char* file_name)
{
    ...
    int read_size = (int)fread (context->source_buffer.source_code_array, sizeof (char), context->source_buffer.buffer_size, file);

    count_n_lines (context);

    if (context->source_buffer.buffer_size != read_size + context->parsed_lines.num_lines - 1)
    {
...
error_t write_commands (context_t* context, const char* file_name)
{
    ...
    fwrite (context->bytecode_container.bytecode, sizeof (int), context->bytecode_container.num_bytecode_elements, file);
